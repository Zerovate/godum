...
void InputPlayerComponent::_on_player_changed(Player *p_prev_player, Player *p_new_player) {
	if (p_prev_player) {
		p_prev_player->disconnect("input_device_changed", Callable(this, "_on_input_device_changed"));
	}
	if (p_new_player) {
		p_new_player->connect("input_device_changed", Callable(this, "_on_input_device_changed"));
	}
