...
void Component::_enter_tree() {
  ...
  if (!actor) {
    if (Engine::get_singleton()->is_editor_hint()) {
      LOG_WARNING("Component %s has no actor", get_name());
    } else {
      CRASH_NOW_MSG(vformat("Component %s has no actor", get_path()));
    }
  } else {
    LOG_DEBUG("%s bind actor %s", get_name(), actor->get_name());
  }
  set_actor(actor);
}
...
Node *Component::get_actor() const { return m_actor; }
...
void Component::_bind_methods() {
  ...
  ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "actor",
                            PROPERTY_HINT_RESOURCE_TYPE, "Node",
                            PROPERTY_USAGE_NONE),
               "set_actor", "get_actor");
  ...
  ADD_SIGNAL(
      MethodInfo("actor_changed",
                 PropertyInfo(Variant::OBJECT, "prev_actor", PROPERTY_HINT_NONE,
                              "", PROPERTY_USAGE_DEFAULT, "Node"),
                 PropertyInfo(Variant::OBJECT, "new_actor", PROPERTY_HINT_NONE,
                              "", PROPERTY_USAGE_DEFAULT, "Node")));
